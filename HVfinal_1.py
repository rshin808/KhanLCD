...
"""
    This class holds the methods for the template for the subwindow of IMS.
    It has a Back, V and I position to choose. 
"""
class IMSN(Temp):
    def __init__(self, name = "IMSN", filename = "IMSN.csv", channel = 1):
        Temp.__init__(self, name, filename)
        # Based on channel name (1,..., channelN)
        self._channel = channel
        self._current_pos = 0
        self._VI_pos = 0
        self._selection = ("IMS", "V", "I")
        self._VI = 0
        ...
        # FIX meas
        self._measI = CHICurrent[self._channel - 1]
        self._setI = CHISet[self._channel - 1]
        
        voltage_string = self.__current_to_string(self._measI)
...
"""
    This class holds the methods for the template for the subwindow of VMS.
    It has a Back, V and I position to choose. 
"""
class VMSN(Temp):
    def __init__(self, name = "VMSN", filename = "VMSN.csv", channel = 1):
        Temp.__init__(self, name, filename)
        # Based on channel name (1,..., channelN)
        self._channel = channel
        self._current_pos = 0
        self._VI_pos = 0
        self._selection = ("VMS", "V", "I")
        self._VI = 0
        self._title = TS(3, 30, 14, "CHANNEL " + str(channel + 1), font14hL)
        ...
        # In terms of DAC voltages
        # The values we want to set to
        self._measV = CHVCurrent[self._channel - 1]
    ...
    def init(self, display):
        voltage_string = "{:.2f}".format(self._measV).zfill(7)
        current_string = "{:.2f}".format(self._setV).zfill(7)
...
"""
    This class holds the methods for the template for the subwindow of VIMeas.
    It has a Back, V and I position to choose. 
"""
class VIMeasN(Temp):
    def __init__(self, name = "VIMeasN", filename = "VIMeasN.csv", channel = 1):
        Temp.__init__(self, name, filename)
        # Based on channel name (1,..., channelN)
        self._channel = channel
        self._current_pos = 0
        self._VI_pos = 0
        self._selection = ("VIMeas")
        self._VI = 0
        self._title = TS(3, 30, 14, "CHANNEL " + str(channel + 1), font14hL)
        ...
        # FIX ChangeCurrent to measured in ALL OF CLASS
        self._measV = CHVCurrent[self._channel - 1]
        self._measI = CHICurrent[self._channel - 1]
        
        voltage_string = self.__voltage_to_string(self._measV)
        current_string = self.__current_to_string(self._measI)
        count = 54
        for v in voltage_string:
            self._V.append(TS(count, 53, 14, v, font14hL))
            count += 10  
    ...
    def init(self, display):
        voltage_string = "{:.2f}".format(self._measV).zfill(7)
        current_string = "{:.2f}".format(self._measI).zfill(7)
        
        # FIX Change color
        self._title.draw_string((0, 0), (175, 223), display)
        for i in range(4):
            self._V[i].update_string(voltage_string[i])
            self._I[i].update_string(current_string[i])
...
VIMeas = VIMeas("VIMeas", "VIMeas1.csv")
VISet = VISet("VISet", "VISet1.csv")
VMS = VMS("VMS", "VMS1.csv")
IMS = IMS("IMS", "IMS1.csv")
VISetN = VISetN("VISetN", "VISetN.csv", 0)
VIMeasN = VIMeasN("VIMeasN", "VIMeasN.csv")
VMSN = VMSN("VMSN", "VMSN.csv")
IMSN = IMSN("IMSN", "IMSN.csv")
#Menu = Menu("Menu", "Menu.csv")
#Setting = Setting("Setting", "Setting.csv")
#SettingN = SettingN("SettingN", "SettingN.csv")
#YorN = YorN("YorN", "YorN.csv");
#Help = Help("Help", "Help.csv");
...
try:
    ...
    while(True):
        ...
        if gpio1 == True:
            print gpio.input(PINS["HV1ONOFF"])
            print CHVCurrent[0]
            print CHVCurrent[0] != 0.0
