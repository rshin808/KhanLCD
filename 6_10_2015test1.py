    ...
    def update_strings(self, adc, dac, bus, display, check_time):
        if (dt.datetime.now() - check_time).total_seconds() >= 0.39: 
            ...
            for count in range(len(output_Vs)):
                ...
                self._ID[count].update_string("{:.4f}".format(output_Is[count]) + "mA ")
...
class IMS(Temp):
    ...
    def update_strings(self, adc, dac, bus, display, check_time):
        if (dt.datetime.now() - check_time).total_seconds()  >= 0.39:
            ...
            for reading in readings:
                if count == 1:
                    self._VD[1].update_string("{:.4f}".format(reading) + "mA ")
                    self._VD[1].draw_string((0, 0), self._back, display)
                elif count == 2:
                    self._VD[2].update_string("{:.4f}".format(reading) + "mA ")
                    self._VD[2].draw_string((0, 0), self._back, display)
                elif count == 6:
                    self._VD[3].update_string("{:.4f}".format(reading) + "mA ")
                    self._VD[3].draw_string((0, 0), self._back, display)
                elif count == 7:
                    self._VD[0].update_string("{:.4f}".format(reading) + "mA ")
                    self._VD[0].draw_string((0, 0), self._back, display)
                count += 1
...
def ramp_default(channel, start, voltage):
    ...
    if check >= 0.02:
        print str(channel) + ":" + str(CHVCurrent[channel]) + "," + str(CHVSet[channel]) + "," + str(voltage)
